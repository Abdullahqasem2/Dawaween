...
module.exports.createTrip = async (request, response) => {
        const { location,startDate,endDate,userId } = request.body;
        try{
            let newTrip =await Trip.create({location,startDate,endDate})
            let user= await User.findByIdAndUpdate({'_id':userId},{$push:{trips:newTrip}})
